...
from PyQt6.QtWidgets import (
    ...
    QLabel, QMessageBox, QGridLayout, QVBoxLayout, QDialog, QTextEdit, QLineEdit
)
...
# srt_overlap_error.py에서 자동 수정 함수 import
from srt_overlap_error import fix_srt_overlaps_and_save
...
class VideoProcessingApp(QWidget):
    ...
    def __init__(self):
        ...
        self.setGeometry(100, 100, 750, 600) # Adjusted initial size for new UI elements
    ...
    def initUI(self):
        ...
        # --- Row 3: Subtitle Overlap Fix Option (NEW) ---
        self.fix_overlap_checkbox = QCheckBox("자막 시간 겹침 자동 수정")
        self.fix_overlap_checkbox.setChecked(True) # 기본값으로 활성화
        layout.addWidget(self.fix_overlap_checkbox, 3, 0, 1, 3)

        # --- Row 4: Upscaling Options ---
        ...
        layout.addWidget(self.upscale_checkbox, 4, 0)

        self.target_resolution_label = QLabel("목표 해상도 (WxH):")
        layout.addWidget(self.target_resolution_label, 4, 1, alignment=Qt.AlignmentFlag.AlignRight)

        self.target_resolution_input = QLineEdit("1920x1080") # 기본 목표 해상도
        self.target_resolution_input.setFixedWidth(100) # 가로 길이 고정
        layout.addWidget(self.target_resolution_input, 4, 2, alignment=Qt.AlignmentFlag.AlignLeft)
        
        # --- Row 5: Start Button ---
        self.start_button = QPushButton("처리 시작")
        self.start_button.clicked.connect(self.start_processing)
        layout.addWidget(self.start_button, 5, 0, 1, 3) # Span 3 columns

        # --- Row 6: Progress Bar ---
        self.progress_bar = QProgressBar()
        self.progress_bar.setValue(0)
        self.progress_bar.setTextVisible(True)
        layout.addWidget(self.progress_bar, 6, 0, 1, 3) # Span 3 columns

        # --- Row 7: Status Label ---
        self.status_label = QLabel("")
        self.status_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(self.status_label, 7, 0, 1, 3) # Span 3 columns
    ...
    def start_processing(self):
        ...
        if use_upscaling:
            try:
                ...
                return
        else: # 업스케일링을 사용하지 않는 경우, 목표 해상도는 실제 사용되지 않지만 0으로 전달
            target_width = 0
            target_height = 0
        # -------------------------------
        ...
        use_padding = self.padding_checkbox.isChecked()
        
        # VideoProcessorManager 인스턴스 생성 (새로운 파라미터 전달)
        self.processor = VideoProcessorManager(
            use_padding, use_upscaling, target_width, target_height
        )
